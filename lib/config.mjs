// mdsite.yml 读写：带注释 YAML（写盘时人类可读），运行时读解析结果。
import { readFile, writeFile, mkdir, access } from 'node:fs/promises'
import path from 'node:path'
import YAML from 'yaml'
import { CONFIG_PATH, MDSITE_HOME } from './paths.mjs'
...
export function buildAnnotatedYaml(cfg) {
  return `# mdsite 配置文件
# 由 \`mdsite init\` 生成；可手动编辑后运行 \`mdsite doctor\` 校验。
...
# 模板映射（相对 ~/.mdsite/templates/，缺省用内置；publish --template <type> 选用）
templates:
  report: ${cfg.templates.report}
  doc: ${cfg.templates.doc}
  dashboard: ${cfg.templates.dashboard}
