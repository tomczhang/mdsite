...
/**
 * 反复调用 genFn 直到产出一个 takenFn 判为「未占用」的候选，最多 max+1 次。
 * 全部碰撞 → 返回 null。takenFn 可同步或异步。
 * 抽成纯函数便于单测 publish 的去碰撞逻辑。
 */
export async function uniqueName(genFn, takenFn, max = 10) {
  for (let i = 0; i <= max; i++) {
    const candidate = genFn()
    if (!(await takenFn(candidate))) return candidate
  }
  return null
}
