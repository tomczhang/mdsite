...
// mdlink 站点标记文件名（写在站点根，唯一标识"这是 mdlink 管理的站点"）。
export const SITE_MARKER = '.mdlink-site.json'

/**
 * 覆盖保护探测（spec：不覆盖非 mdlink 管理的 gh-pages 站点）。
 * 返回 'absent'（分支不存在，安全新建）
 *     | 'mdlink'（含专用 marker，安全续写）
 *     | 'foreign'（分支有内容但无 marker，需 --force）。
 * 用专用 marker 而非 pages.json —— 避免无关站点恰好有 pages.json 被误判为 mdlink。
 */
