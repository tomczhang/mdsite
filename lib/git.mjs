...
import { GITIGNORE_ENTRIES, PAGES_JSON_REL } from './paths.mjs'

export function remoteHttpsUrl(repo) {
  return `https://github.com/${repo}.git`
}

// askpass 脚本放在系统临时目录，【不能】放在工作区下——否则会在 clone 前把
// 目标目录(~/.mdsite)弄成非空，导致 clone 失败。
const ASKPASS = path.join(os.tmpdir(), 'mdsite-askpass.sh')
