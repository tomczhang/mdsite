# AGENTS.md — mdsite 项目背景与约定

## 是什么
mdsite = 把 Markdown / 对话内容一键转成「云端可分享的精美 HTML 报告」，部署到**用户自己的 GitHub Pages**（BYO-host）。形态 = skill + CLI（Node ESM，node>=20）。
...
## 关键决策
- **部署**：GitHub Pages 分支部署只伺服仓库根或 /docs，不支持 /src ⇒ 用 **gh-pages 分支、站点在工作区根**。`~/.mdsite` 工作树 = gh-pages 分支；`mdsite.yml`/`templates/`/`.cache/` 用 `.gitignore` 排除，不进站点。
...
## 结构
- `bin/mdsite.mjs` 入口路由｜`cmds/` 子命令（init/publish/doctor/serve/template）｜`lib/` 纯逻辑
