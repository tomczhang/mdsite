...
- [x] 1.1 初始化 `package.json`（name=mdlink, type=module, bin.mdlink=bin/mdlink.mjs, engines.node>=20；deps: zx + marked；devDeps: vitest + @playwright/test） — `package.json`
...
- [x] 2.1 `lib/paths.mjs`：路径常量（MDLINK_HOME 默认 ~/.mdlink、SRC_DIR、PAGES_JSON、模板目录、CLI 内嵌模板目录） — `lib/paths.mjs`
...
- [x] 2.5 `lib/config.mjs`：`mdlink.yml` 读写（带注释 YAML 生成）+ `defaultConfig`（remote.repo/branch、deploy_url=`https://<user>.github.io/<repo>/`、模板映射、storage） — `lib/config.mjs`
...
- [x] 3.2 `lib/git.mjs`：HTTPS 认证（优先 gh 助手/GIT_ASKPASS/一次性 http.extraHeader，临时嵌 URL 兜底）+ clone/commit/push；`fetch→rebase`，仅 pages.json 冲突时调 mergePagesJson 自动合并、其它冲突 abort — `lib/git.mjs`
- [x] 3.3 **token 不持久化**：临时嵌入的凭证 push 后还原/清除 remote URL，确保不留在 .git/config、worktree config、日志；日志脱敏（`x-access-token:***`） — `lib/git.mjs` `lib/log.mjs`
- [x] 3.4 gh-pages 发布机制：把工作区 `src/` 内容映射推送到 gh-pages 分支根（独立 worktree 路径，最稳实现） — `lib/git.mjs`
- [x] 3.5 **覆盖保护**：写 gh-pages 前探测 mdlink 标记(pages.json/index)，非 mdlink 站点 BLOCK，支持 `--force` 显式覆盖 — `lib/gh.mjs`/`cmds/publish.mjs`
...
- [x] 5.1 `bin/mdlink.mjs`：入口，zx 注入 $/fs/path，参数解析 + 路由 init/publish/doctor/serve/template，未知命令报用法 — `bin/mdlink.mjs`
- [x] 5.2 `cmds/init.mjs`：whoami/凭证检查 → repoView/Create → enablePages(gh-pages) → 准备本地工作区(git+src+pages.json) → 写 mdlink.yml → 铺首页 — `cmds/init.mjs`
- [x] 5.3 `cmds/publish.mjs`：输入 md/html 分流→render→归档 `src/<category>/<date>/<slug>.html`→prepend pages.json→重渲首页→commit→push gh-pages→输出真实 URL；category kebab-case 校验 — `cmds/publish.mjs`
...
- [x] 5.5 `cmds/serve.mjs`：本地静态伺服工作区 src/ — `cmds/serve.mjs`
...
- [x] 7.3a 凭证泄露自检：publish 后 grep `.git/config`/worktree config/日志确认无明文 token；slug 碰撞用例（同标题同日两篇）单测 — `tests/`
