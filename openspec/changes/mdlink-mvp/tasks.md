...
- [x] 1.1 初始化 `package.json`（name=mdsite, type=module, bin.mdsite=bin/mdsite.mjs, engines.node>=20；deps: zx + marked；devDeps: vitest + @playwright/test） — `package.json`
...
- [x] 2.1 `lib/paths.mjs`：路径常量（MDSITE_HOME 默认 ~/.mdsite、SRC_DIR、PAGES_JSON、模板目录、CLI 内嵌模板目录） — `lib/paths.mjs`
...
- [x] 3.5 **覆盖保护**：写 gh-pages 前探测 mdsite 标记(pages.json/index)，非 mdsite 站点 BLOCK，支持 `--force` 显式覆盖 — `lib/gh.mjs`/`cmds/publish.mjs`
...
- [x] 5.1 `bin/mdsite.mjs`：入口，zx 注入 $/fs/path，参数解析 + 路由 init/publish/doctor/serve/template，未知命令报用法 — `bin/mdsite.mjs`
- [x] 5.2 `cmds/init.mjs`：whoami/凭证检查 → repoView/Create → enablePages(gh-pages) → 准备本地工作区(git+src+pages.json) → 写 mdsite.yml → 铺首页 — `cmds/init.mjs`
