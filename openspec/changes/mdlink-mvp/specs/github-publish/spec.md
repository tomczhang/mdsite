...
### Requirement: 不覆盖非 mdsite 管理的已有 gh-pages 站点
当目标 repo 已存在一个 `gh-pages` 分支/站点时，工具 SHALL 在写入前判定它是否由 mdsite 管理——依据是分支根存在**专用标记文件 `.mdsite.json`**（不能仅凭 `pages.json`，以免无关站点恰好有同名文件被误判）。**仅当确认是 mdsite 管理的站点时**才继续发布；否则 SHALL 以明确错误 BLOCK，并要求用户显式 `--force`（或独立的 import 流程）后才覆盖。

#### Scenario: 已存在的非 mdsite 站点被保护
- **WHEN** 目标 repo 的 `gh-pages` 分支存在内容但缺少 `.mdsite.json` 标记（即便它恰好有 `pages.json`）
- **THEN** 工具拒绝写入、以清晰错误退出，提示这会覆盖既有站点，需 `--force` 或单独 import 才能继续

#### Scenario: mdsite 管理的站点正常续写
- **WHEN** 目标 `gh-pages` 含 `.mdsite.json` 标记
- **THEN** 工具识别为自己管理的站点，正常追加发布、合并 `pages.json`，不需 `--force`

#### Scenario: 显式 force 覆盖
- **WHEN** 用户带 `--force` 对一个非 mdsite 站点发布
- **THEN** 工具按用户显式意图覆盖，并在输出中明确告知发生了覆盖

### Requirement: 仓库主页可读（默认分支 = gh-pages + README）
对**本次由工具新建**的 repo，`init` 完成后其 GitHub 主页 SHALL 呈现有意义的内容，而非一个空的 `main`/README：默认分支 SHALL 设为 `gh-pages`，并 SHALL 删除 auto-init 产生的空 `main`；gh-pages 站点根 SHALL 含一份 `README.md`（GitHub 在默认分支主页渲染，含指向 live 站点的链接）。
**对复用的已有 repo**，工具 SHALL NOT 修改其默认分支、SHALL NOT 删除其 `main`（那可能是用户真实代码，删除即数据丢失）；只部署 gh-pages 并写入站点 `README.md`。

#### Scenario: 新建 repo 的主页不再是空 README
- **WHEN** 工具新建一个 repo 并 `init` 完成后访问其 GitHub 主页
- **THEN** 默认分支是 `gh-pages`，主页渲染站点 `README.md`（含 live 链接），不存在只有空 README 的 `main`

#### Scenario: 复用已有 repo 不动其 main
- **WHEN** 对一个已存在、且 `main` 含用户内容的 repo 执行 `init`
- **THEN** 工具不修改其默认分支、不删除其 `main`，只在 gh-pages 上部署站点
...
工具进行 git 远端操作时 SHALL 使用用户自己的 GitHub 凭证经 HTTPS 认证，**SHALL NOT 依赖 SSH 作为默认路径**。认证方式 SHALL 优先选用不落盘的方案：`gh` 凭证助手 / `GIT_ASKPASS` / 一次性 HTTP `Authorization` header（`http.extraHeader`）。**`GITHUB_TOKEN` SHALL NOT 被持久化**到任何 git 配置或文件中——具体地：token 不得长期写入 `~/.mdsite/.git/config`、任何临时 worktree 的 config、commit 内容、或日志/错误输出。
...
- **WHEN** 一次 publish 完成（含成功或失败路径）后检查 `~/.mdsite/.git/config`、临时 worktree config 与持久 remote URL
...
#### Scenario: 一条命令出链接
- **WHEN** 用户执行 `mdsite publish <input> --title T --category report --summary S`
